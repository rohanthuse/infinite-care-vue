...
-- Fix existing system users without auth_user_id
DO $$
DECLARE
  system_user_record RECORD;
  sync_result json;
BEGIN
  FOR system_user_record IN 
    SELECT su.*, suo.organization_id, suo.role as org_role
    FROM system_users su
    JOIN system_user_organizations suo ON su.id = suo.system_user_id
    WHERE su.auth_user_id IS NULL
  LOOP
    -- Sync each system user to their organization
    SELECT sync_system_user_to_organization(
      system_user_record.id,
      system_user_record.organization_id,
      system_user_record.org_role
    ) INTO sync_result;
    
    RAISE NOTICE 'Synced user %: %', system_user_record.email, sync_result;
  END LOOP;
END;
$$;