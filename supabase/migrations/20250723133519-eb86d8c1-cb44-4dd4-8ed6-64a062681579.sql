...
-- Create get_staff_profile_by_auth_user_id function if it doesn't exist
...
-- Fix staff_documents table RLS policies to allow staff to access their own documents
DROP POLICY IF EXISTS "Staff can manage their own documents" ON public.staff_documents;
DROP POLICY IF EXISTS "Branch staff can view staff documents" ON public.staff_documents;
...
CREATE POLICY "Branch staff can view staff documents"
  ON public.staff_documents
  FOR SELECT
  USING (staff_id IN (
    SELECT s.id FROM public.staff s
    WHERE s.branch_id IN (
      SELECT s2.branch_id FROM public.staff s2 WHERE s2.auth_user_id = auth.uid()
      UNION
      SELECT ab.branch_id FROM public.admin_branches ab WHERE ab.admin_id = auth.uid()
    )
  ));

-- Update documents table RLS policies to allow staff access
DROP POLICY IF EXISTS "Users can view documents in their branch" ON public.documents;
DROP POLICY IF EXISTS "Users can upload documents in their branch" ON public.documents;
DROP POLICY IF EXISTS "Users can update documents in their branch" ON public.documents;
DROP POLICY IF EXISTS "Users can delete documents in their branch" ON public.documents;
...
CREATE POLICY "Users can view documents in their branch"
  ...
  USING (
    auth.uid() IS NOT NULL AND (
      -- Admin access
      branch_id IN (
        SELECT ab.branch_id FROM public.admin_branches ab 
        WHERE ab.admin_id = auth.uid()
      ) OR
      -- Staff access to their branch documents
      branch_id IN (
        SELECT s.branch_id FROM public.staff s 
        WHERE s.auth_user_id = auth.uid()
      ) OR
...
CREATE POLICY "Users can upload documents in their branch"
  ...
  WITH CHECK (
    auth.uid() IS NOT NULL AND (
      ...
      -- Staff can upload to their branch
      branch_id IN (
        SELECT s.branch_id FROM public.staff s 
        WHERE s.auth_user_id = auth.uid()
      ) OR
