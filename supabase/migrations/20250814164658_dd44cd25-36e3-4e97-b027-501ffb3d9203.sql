-- Fix the type casting error in reset_system_user_password_with_session function
...
BEGIN
  ...
  IF auth_user_id IS NULL THEN
    ...
    -- Get the user's role from system_user_roles and assign it with proper role mapping
    INSERT INTO public.user_roles (user_id, role) 
    SELECT auth_user_id, 
      CASE sur.role::text
        WHEN 'super_admin' THEN 'super_admin'::app_role
        WHEN 'tenant_manager' THEN 'branch_admin'::app_role  
        WHEN 'support_admin' THEN 'super_admin'::app_role
        WHEN 'analytics_viewer' THEN 'super_admin'::app_role
        ELSE 'super_admin'::app_role
      END
