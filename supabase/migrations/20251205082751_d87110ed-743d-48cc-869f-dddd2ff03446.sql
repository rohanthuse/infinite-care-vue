-- Fix the RPC function with corrected column names
...
BEGIN
  -- Validate system admin via session token (FIXED: correct column names)
  IF p_session_token IS NOT NULL THEN
    SELECT EXISTS(
      SELECT 1 FROM system_sessions ss
      JOIN system_users su ON ss.system_user_id = su.id
