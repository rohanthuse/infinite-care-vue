-- Fix the create_third_party_user_with_password function to use correct enum values
...
BEGIN
  ...
  -- Map request_for to access_type enum (use correct enum values: client, staff, both)
  v_access_type := CASE 
    WHEN v_request_for = 'client' THEN 'client'::third_party_access_type
    WHEN v_request_for = 'staff' THEN 'staff'::third_party_access_type
    WHEN v_request_for = 'both' THEN 'both'::third_party_access_type
    ELSE 'client'::third_party_access_type
  END;
