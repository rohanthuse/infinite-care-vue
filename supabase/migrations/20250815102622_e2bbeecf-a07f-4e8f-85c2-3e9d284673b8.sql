-- Fix enum validation error in system users RPC functions
...
BEGIN
  ...
  -- Check if session is valid and user has appropriate role (using valid enum values)
  ...
  -- Return system users with their organizations (using valid enum default)
  RETURN QUERY
  SELECT 
    su.id,
    su.email,
    ...
    su.last_login_at,
    su.created_at,
    COALESCE(sur.role, 'support_admin') as role,
    COALESCE(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
