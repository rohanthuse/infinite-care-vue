...
-- 0) Ensure pgcrypto is available in the extensions schema (safe to run repeatedly)
CREATE SCHEMA IF NOT EXISTS extensions;
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- 1) Keep helper functions consistent (do not rely only on public in nested calls)
...
-- 2) Recreate the main RPC with explicit pgcrypto references and preserve the DEFAULT param
...
BEGIN
  ...
  -- Create user with hashed password (explicitly use pgcrypto in extensions schema)
