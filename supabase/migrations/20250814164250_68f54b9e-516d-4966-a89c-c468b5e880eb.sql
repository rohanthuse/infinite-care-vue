-- Fix the system user password reset function by removing non-existent column references
...
DECLARE
  ...
  result json;
BEGIN
  -- Validate session token and get admin user info (removed ss.is_active check)
  ...
  -- Update only existing columns in system_users (removed non-existent password_reset_at and password_reset_by)
