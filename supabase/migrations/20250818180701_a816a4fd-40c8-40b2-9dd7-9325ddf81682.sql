-- Fix Multi-Tenancy Data Isolation - Final Implementation with Correct Type Values

-- Step 1: Allow NULL organization_id for template records
...
-- Step 2: Create template records for communication_types
...
-- Step 5: Create template records for expense_types with correct type values
...
-- Step 7: Remove duplicate data while keeping one per organization  
...
-- Step 8: Create auto-seeding trigger for new organizations
...
-- Step 9: Seed existing organizations that need parameters
DO $$
DECLARE
  org_record RECORD;
BEGIN
  FOR org_record IN SELECT id FROM organizations LOOP
    -- Always try to seed - function handles conflicts
