...
DROP FUNCTION IF EXISTS public.system_logout(text);
...
-- Create the system authentication function
CREATE FUNCTION public.system_authenticate(
  p_email text,
  p_password text,
...
-- Create the system session validation function
CREATE FUNCTION public.system_validate_session(
...
-- Create the system logout function
CREATE FUNCTION public.system_logout(
  p_session_token text
)
RETURNS jsonb
LANGUAGE plpgsql
