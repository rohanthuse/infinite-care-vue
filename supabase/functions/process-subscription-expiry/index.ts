...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      console.error('[process-subscription-expiry] Database error:', error);
      throw error;
    }

    console.log('[process-subscription-expiry] Processing complete:', data);
    ...
    console.error('[process-subscription-expiry] Edge function error:', error);
    return new Response(
