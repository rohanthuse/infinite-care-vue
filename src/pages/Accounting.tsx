...
const Accounting = () => {
  ...
  const [activeNavTab, setActiveNavTab] = useState("accounting");
  const decodedBranchName = decodeURIComponent(branchName || "Med-Infinite Branch");

  useEffect(() => {
    document.title = `Accounting | ${decodedBranchName}`;
  }, [decodedBranchName]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-50 to-white">
      ...
      <main className="flex-1 px-4 md:px-8 pt-4 pb-8 md:py-6 w-full max-w-[1600px] mx-auto">
        ...
        <div className="mt-6">
          <AccountingTab branchId={id} branchName={decodedBranchName} />
        </div>
      </main>
    </div>
  );
