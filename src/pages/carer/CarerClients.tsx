...
const CarerClients: React.FC = () => {
  ...
  const filteredClients = mockClients.filter(client => {
    return (
      ...
      client.careNeeds.some(need => need.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  });
  
  // Handle view client
  const handleViewClient = (clientId: string) => {
    navigate(`/carer-dashboard/client/${clientId}`);
  };
            ...
            >
              ...
              <CardContent>
                <div className="space-y-3">
                  {/* Note: We don't display the full address for privacy reasons */}
                  <div>
                ...
                <div className="flex gap-2 mt-4">
                  <Button size="sm" className="w-full" onClick={() => handleViewClient(client.id)}>View Details</Button>
                </div>
              </CardContent>
            </Card>
          ))
