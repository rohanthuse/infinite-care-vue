...
const ClientDashboard = () => {
  ...
  const { user, loading, isAuthenticated, isClientRole } = useClientAuth();
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated || !isClientRole) {
        console.log('[ClientDashboard] Authentication failed, redirecting to login');
        navigate("/client-login", { replace: true });
        return;
      }
      
      console.log('[ClientDashboard] Client authenticated:', {
        userId: user?.id,
        email: user?.email,
        isAuthenticated,
        isClientRole
  ...
  }, [loading, isAuthenticated, isClientRole, navigate, user]);
