...
import { ClientContactSidebarFixed } from "@/components/client/ClientContactSidebarFixed";
import { ClientMessageListFixed } from "@/components/client/ClientMessageListFixed";
import { ClientMessageViewFixed } from "@/components/client/ClientMessageViewFixed";
import { ClientMessageComposerFixed } from "@/components/client/ClientMessageComposerFixed";

// Memoize components to prevent unnecessary re-renders
const MemoizedContactSidebar = memo(ClientContactSidebarFixed);
const MemoizedMessageList = memo(ClientMessageListFixed);
const MemoizedMessageView = memo(ClientMessageViewFixed);  
const MemoizedMessageComposer = memo(ClientMessageComposerFixed);

const ClientMessagesFixed = () => {
  ...
  const handleContactSelect = (contactId: string) => {
    console.log('[ClientMessagesFixed] Contact selected:', contactId);
  ...
  const handleMessageSelect = (messageId: string) => {
    console.log('[ClientMessagesFixed] Message selected:', messageId);
  ...
  const handleComposeClick = () => {
    console.log('[ClientMessagesFixed] Compose clicked');
    setShowComposer(true);
    setSelectedMessageId(null);
  };
  
  const handleSendMessage = () => {
    console.log('[ClientMessagesFixed] Message sent');
    setShowComposer(false);
  ...
  const handleReply = () => {
    console.log('[ClientMessagesFixed] Reply clicked');
    setShowComposer(true);
  };
...
export default ClientMessagesFixed;
