...
export const useFluidOutputRecords = (clientId: string, date?: string) => {
  return useQuery({
    queryKey: ['fluid-output-records', clientId, date],
    queryFn: async () => {
      ...
      return data as FluidOutputRecord[];
    },
    enabled: !!clientId,
  });
};

export const useAddFluidOutputRecord = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (record: Omit<FluidOutputRecord, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to add fluid output record',
        variant: 'destructive',
      });
    },
  });
...
export const useUpdateFluidOutputRecord = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ id, ...updates }: Partial<FluidOutputRecord> & { id: string }) => {
      const { data, error } = await (supabase as any)
        .from('fluid_output_records')
        .update(updates)
        .eq('id', id)
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to update fluid output record',
        variant: 'destructive',
      });
    },
  });
...
export const useDeleteFluidOutputRecord = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to delete fluid output record',
        variant: 'destructive',
      });
    },
  });
...
export const useFluidOutputSummary = (clientId: string, date: string) => {
  return useQuery({
    queryKey: ['fluid-output-summary', clientId, date],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from('fluid_output_records')
        .select('amount_ml')
