      ...
      try {
        ...
        if (typedRateSchedules.length === 0) {
          ...
          if (rateAssignments && rateAssignments.length > 0) {
            ...
            // Filter rate assignments to prefer those matching the client's configured authority
            const preferredAuthorityId = billingConfig.authorityId;
            let filteredAssignments = rateAssignments;
            
            if (preferredAuthorityId) {
              const authorityMatchedRates = rateAssignments.filter(
                (r: any) => r.authority_id === preferredAuthorityId
              );
              if (authorityMatchedRates.length > 0) {
                console.log(`[BulkInvoiceGeneration] Found ${authorityMatchedRates.length} rates matching authority ${preferredAuthorityId}`);
                filteredAssignments = authorityMatchedRates;
              } else {
                console.warn(`[BulkInvoiceGeneration] No rates match authority ${preferredAuthorityId}, using all ${rateAssignments.length} assignments`);
              }
            }
            
            // Convert rate assignments to rate schedule format with ALL required fields for VisitBillingCalculator
            typedRateSchedules = filteredAssignments.map((assignment: any) => {
              const schedule = {
                id: assignment.id,
