  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select(`
          *,
          staff:staff!inner(first_name, last_name, specialization),
          clients:clients!inner(first_name, last_name)
        `)
      ...
      // Transform the data to include person names and roles
      const transformedData = data?.map(record => ({
        ...record,
        person_name: record.person_type === 'staff' 
          ? `${record.staff?.first_name} ${record.staff?.last_name}`
          : `${record.clients?.first_name} ${record.clients?.last_name}`,
        person_role: record.person_type === 'staff' 
          ? record.staff?.specialization || 'Staff'
          : 'Client',
      })) || [];

      // Apply client-side filters for search and roles
