...
import { LayoutDashboard, Workflow, ListChecks, Users, Calendar, Star, MessageSquare, Pill, DollarSign, FileText, ClipboardCheck, Bell, ClipboardList, FileUp, Folder, UserPlus, BarChart4, Settings, Activity, Briefcase, PanelLeft, Paperclip, ChevronDown, Menu, Search, Grid, Plus, UserPlus2, FileSignature, CalendarPlus, Contact, UserRoundPlus } from "lucide-react";
import { cn } from "@/lib/utils";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuGroup, DropdownMenuLabel, DropdownMenuSub, DropdownMenuSubTrigger, DropdownMenuSubContent, DropdownMenuPortal } from "@/components/ui/dropdown-menu";
...
export const TabNavigation = ({
  ...
  const filteredTabs = allTabs.filter(tab => tab.label.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="w-full">
      <div className="flex flex-col space-y-4">
        {!hideActionsOnMobile && <div className="flex items-center justify-between md:hidden bg-white p-3 rounded-lg shadow-sm">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-2 z-30 md:hidden">
          <div className="flex justify-around">
            ...
            return <Button key={tab.value} variant="ghost" size="sm" className={cn("flex flex-col items-center justify-center gap-1 h-auto px-1 py-2 rounded-lg", isActive ? "bg-blue-50 text-blue-600" : "text-gray-500")} onClick={() => onChange(tab.value)}>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className={cn("flex flex-col items-center justify-center gap-1 h-auto px-1 py-2 rounded-lg", activeTab !== "dashboard" && !primaryTabs.slice(0, 5).some(t => t.value === activeTab) ? "bg-blue-50 text-blue-600" : "text-gray-500")}>
              ...
              <PopoverContent className="w-64" align="end" sideOffset={5}>
                <div className="p-2">
                  <Input placeholder="Search modules..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="mb-2" />
                </div>
                <div className="max-h-[50vh] overflow-y-auto p-1">
                  {filteredTabs.length > 0 ? <>
                      {primaryTabs.slice(5).map(tab => {
                    const Icon = tab.icon;
                    return <Button key={tab.value} variant="ghost" className={cn("w-full justify-start text-left py-2 px-3 rounded-md my-1", tab.value === activeTab ? "bg-blue-50 text-blue-600" : "")} onClick={() => {
                      onChange(tab.value);
                      setSearchTerm("");
                    }}>
                      ...
                      {secondaryTabGroups.map(group => <div key={group.label} className="py-1">
                          <div className="px-3 py-1 text-xs font-semibold text-gray-500">
                            {group.label}
                          </div>
                          {group.items.filter(item => filteredTabs.some(tab => tab.value === item.value)).map(item => {
                      const Icon = item.icon;
                      return <Button key={item.value} variant="ghost" className={cn("w-full justify-start text-left py-2 px-3 rounded-md my-1", item.value === activeTab ? "bg-blue-50 text-blue-600" : "")} onClick={() => {
                        onChange(item.value);
                        setSearchTerm("");
                      }}>
                                  <Icon className="h-4 w-4 mr-2" />
            ...
            <div className="w-full overflow-x-auto hide-scrollbar bg-white border border-gray-100 rounded-xl shadow-sm">
              <Tabs value={activeTab} onValueChange={onChange} className="w-full">
                <TabsList className="bg-white p-1 rounded-xl w-full justify-start">
                  {primaryTabs.map(tab => {
                  const Icon = tab.icon;
                  return <TabsTrigger key={tab.value} value={tab.value} className={cn("flex items-center gap-2 px-4 py-2 rounded-lg data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600", "transition-all duration-200 flex-shrink-0")}>
            ...
            <div className="flex items-center gap-2">
              <Popover>
                ...
                <PopoverContent className="w-80 p-0" align="end">
                  <div className="p-3 border-b">
                    <Input placeholder="Search modules..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full" />
                  </div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 p-1 bg-white border border-gray-200 shadow-lg rounded-lg">
                  {secondaryTabGroups.map(group => <DropdownMenuSub key={group.label}>
                      ...
                      <DropdownMenuPortal>
                        ...
                        return <DropdownMenuItem key={item.value} className={cn("py-2 px-3 rounded-md my-1 text-sm cursor-pointer", isActive ? "bg-blue-50 text-blue-600" : "hover:bg-gray-50")} onClick={() => onChange(item.value)}>
                                <Icon className="h-4 w-4 mr-2" />
                                <span>{item.label}</span>
                              </DropdownMenuItem>;
                      })}
