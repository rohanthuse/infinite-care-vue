...
import { 
  useMedicationTrendData, 
  useMedicationTypeDistribution, 
  useTimeOfDayDistribution 
} from "@/hooks/useMedicationChartData";
...
export const MedChartData: React.FC<MedChartDataProps> = ({ patientId, viewType, branchId }) => {
  // Fetch real data using our new hooks
  const { data: medicationTrendData = [], isLoading: trendLoading } = useMedicationTrendData(branchId);
  const { data: medicationTypeData = [], isLoading: typeLoading } = useMedicationTypeDistribution(branchId);
  const { data: timeOfDayData = [], isLoading: timeLoading } = useTimeOfDayDistribution(branchId);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      {trendLoading ? (
        <div className="col-span-1 lg:col-span-2">
          <LoadingChart title="Medication Administration Trend" />
        </div>
      ) : (
      ...
      {typeLoading ? (
        <LoadingChart title="Medication Type Distribution" />
      ) : (
        <Card>
          <CardHeader>
      ...
      {timeLoading ? (
        <LoadingChart title="Time of Day Distribution" />
      ) : (
        <Card>
          <CardHeader>
