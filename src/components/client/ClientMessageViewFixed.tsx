...
import { useClientThreadMessagesFixed } from "@/hooks/useClientMessagingFixed";

interface ClientMessageViewFixedProps {
  messageId: string; // This is actually threadId
  onReply: () => void;
}

export const ClientMessageViewFixed = ({ messageId: threadId, onReply }: ClientMessageViewFixedProps) => {
  const { data: messages = [], isLoading, error } = useClientThreadMessagesFixed(threadId);
