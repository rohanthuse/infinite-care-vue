...
export function AdminClientDetailsDialog({
  ...
  const handleRemoveWarning = (index: number) => {
    const currentWarnings = form.getValues("warnings") || [];
    form.setValue("warnings", currentWarnings.filter((_, i) => i !== index));
  };
  ...
  const handleAddOccasion = () => {
    const currentOccasions = form.getValues("important_occasions") || [];
    form.setValue("important_occasions", [...currentOccasions, { occasion: "", date: "" }]);
  };

  const handleRemoveOccasion = (index: number) => {
    const currentOccasions = form.getValues("important_occasions") || [];
    form.setValue("important_occasions", currentOccasions.filter((_, i) => i !== index));
  };

  const warnings = form.watch("warnings") || [];
  const instructions = form.watch("instructions") || [];
  const occasions = form.watch("important_occasions") || [];
