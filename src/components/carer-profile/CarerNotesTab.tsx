...
import { AddNoteDialog } from "@/components/staff/dialogs/AddNoteDialog";
import { EditNoteDialog } from "@/components/staff/dialogs/EditNoteDialog";
import { DeleteNoteDialog } from "@/components/staff/dialogs/DeleteNoteDialog";
import { useStaffNotes, useCreateStaffNote, useUpdateStaffNote, useDeleteStaffNote, StaffNote } from "@/hooks/useStaffNotes";

interface CarerNotesTabProps {
  carerId: string;
}

export const CarerNotesTab: React.FC<CarerNotesTabProps> = ({ carerId }) => {
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedNote, setSelectedNote] = useState<StaffNote | null>(null);
  
  const { data: notes = [], isLoading } = useStaffNotes(carerId);
  const createNoteMutation = useCreateStaffNote();
  const updateNoteMutation = useUpdateStaffNote();
  const deleteNoteMutation = useDeleteStaffNote();
  ...
  const handleDeleteNote = React.useCallback((note: StaffNote) => {
    setSelectedNote(note);
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-2 bg-gradient-to-r from-blue-50 to-white">
          ...
          <CardDescription>Notes and observations for this staff member</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-4">
            {notes.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">No notes available for this staff member</p>
              </div>
            ) : (
              notes.map((note) => (
                <div 
